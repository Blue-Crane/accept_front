...
import { ColorSwatch, Tip } from '@ui/basics';
...
const BarPlot: FC<{
  ...
  const barsWidth = hideRowLabels ? 320 : 300;
  const padding = (barsWidth / (data.length + 1)) * PADDING;
  const width =
    (barsWidth - padding * (data.length + 1)) / data.length;
